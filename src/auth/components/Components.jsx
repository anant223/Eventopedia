...
const SocialBtn = ({ icon, label, onClick, className = "" }) => {
  return (
    <button
      type="button"
      onClick={onClick}
...
const Field = forwardRef(({ label, type = "text", ...rest }, ref) => {
  return (
    <div className="flex flex-col gap-1.5">
      ...
      <input
        ref={ref}
        type={type}
        className="h-11 bg-[#faf9f7] border border-black/[0.13] rounded-[11px] px-3.5 text-[14px] text-[#1a1814] placeholder:text-[#8a8480]/60 outline-none transition-all focus:border-[#D85A30] focus:bg-white focus:shadow-[0_0_0_3px_rgba(216,90,48,0.12)]"
        {...rest}
      />
    </div>
  );
