...
const LikeButton = ({ eventId }) => {
  ...
  return (
    <button
      onClick={toggleLike}
      disabled={!user}
      className="text-gray-300 hover:text-white transition-colors duration-200 bg-black/20 hover:bg-black/40 backdrop-blur-sm p-2 rounded-full flex gap-1 items-center disabled:opacity-50"
    >
      {isLiked ? (
        <svg
      ...
      <span className="text-sm font-medium">{eventLikes?.count}</span>
    </button>
  );
};
