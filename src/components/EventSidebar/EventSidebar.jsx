...
const EventSidebar = () => {
  ...
  return (
    <aside className="h-full bg-gray-800 border-r-[0.1px] border-gray-700 text-white top-20 z-30 fixed font-roboto w-[260px] flex flex-col justify-between">
      <div>
        <nav className="flex flex-col pl-12">
          <ul className="space-y-2 py-4">
            {sidebarMenuItems.map((item) => (
              <li key={item.label}>
                <button
                  onClick={() => handelmenuItemsClick(item.href)}
                  className={`flex items-center gap-3 px-4 py-2 text-gray-100 hover:bg-gray-500 transition-colors w-full rounded-e ${
